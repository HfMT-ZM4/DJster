...
public class Factorize extends MaxObject {
    ...
    private void factor(int number)  {
        //To preserve backwards compatibility with how the JavaScript code works
        //the corner case of 1 will be handled initially.
        if(number == 1){
            outlet(0,1);
            return;
        }
    ...
    private Vector<Integer> primeFactorList(int number) throws IllegalArgumentException {
        ...
        //Here is a difference from JavaSource that checked < 1, but we have handled that case
        //already.
        if(number < 2){
